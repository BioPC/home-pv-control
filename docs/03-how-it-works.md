...
HPVC mirrors the configured HBC strategy entity's live option list into its strategy helpers. The Charge strategy is automatically managed and shown read-only: HPVC always uses `Charge` when that exact option exists. Balanced and Expensive remain user-selectable. Balanced prefers `Dynamic`, while Expensive prefers `Self-consumption` when available. If a preferred option does not exist, HPVC preserves a valid fallback, uses HBC's current valid strategy, or selects the first available option.
